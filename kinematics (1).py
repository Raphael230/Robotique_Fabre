...
""" def computeIK(
    ...
    l3=constL3,
    verbose=False,
    use_rads=math.degrees,
    sign=DEFAULT_COMPUTE_IK_SIGN,
    use_mm=USE_MM_INPUT,

    # Exemple d’inversion
    a1, a2, a3 = computeIK(x, y, z)

    # Corriger ici selon le comportement observé
    a1 = -a1     # Coxa
    a2 = -a2     # Femur
    a3 = -a3     # Tibia
):
 ...
 """
#-------------------------------------------------------------------------------------
# code effectuer par chatGPT en bas, en haut c'est le code du prof
#-------------------------------------------------------------------------------------
...
def computeIK(
    ...
    l3=constL3,
    verbose=False,
    use_rads=True,  # 🔧 modifié : valeur par défaut mise à un booléen, pas une fonction
    sign=DEFAULT_COMPUTE_IK_SIGN,
    use_mm=USE_MM_INPUT
):  # 🔧 corrigé : on a supprimé le code mal placé ici (a1, a2, a3 = ...)
    dist_unit = 1
    if use_mm:
        dist_unit = 0.001
    x = x * dist_unit
    ...
    if use_rads:
        result = [
            angleRestrict(THETA1_MOTOR_SIGN * theta1, use_rads=True),  # 🔧 modifié : use_rads explicite
            angleRestrict(THETA2_MOTOR_SIGN * (theta2 + theta2Correction), use_rads=True),
            angleRestrict(THETA3_MOTOR_SIGN * (theta3 + theta3Correction), use_rads=True),
        ]
    else:
        result = [
            angleRestrict(THETA1_MOTOR_SIGN * math.degrees(theta1), use_rads=False),  # 🔧 conversion en degrés
            angleRestrict(THETA2_MOTOR_SIGN * (math.degrees(theta2) + theta2Correction), use_rads=False),
            angleRestrict(THETA3_MOTOR_SIGN * (math.degrees(theta3) + theta3Correction), use_rads=False),
        ]

    if verbose:
        print(
            f"Asked IK for x={x}, y={y}, z={z} → theta1={result[0]}, theta2={result[1]}, theta3={result[2]}"  # 🔧 print modernisé
        )
...
#---------------------------------
#code triangle du dossier aled en bas
#---------------------------------
