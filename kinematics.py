...
def triangle(x, z, h, w, t, leg_id=None):
    ...
    # ajouter true or apres if pour tourner
    if leg_id is None:
...
def update_moving_leg():
        target_x = p.readUserDebugParameter(controls["move_x"])
        target_y = p.readUserDebugParameter(controls["move_y"])
        target_z = p.readUserDebugParameter(controls["move_z"])